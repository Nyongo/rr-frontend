...
interface BusActivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export const BusActivationDialog = ({ 
  isOpen, 
  onClose, 
  busRegistration, 
  onConfirm 
}: BusActivationDialogProps) => {
  ...
  const [activationItems, setActivationItems] = useState<ActivationItem[]>([
    {
      id: 'bus',
      label: 'Bus',
      description: 'Bus will be marked as available for upcoming trips and route assignments.',
      status: 'pending'
    }
  ]);
  ...
  const handleActivate = async () => {
    ...
    toast({
      title: "Bus Activated!",
      description: `Bus ${busRegistration} has been successfully activated and is now available for the upcoming trips.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Activating Bus {busRegistration}</DialogTitle>
            <DialogDescription>
              Please wait while we activate the bus and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-green-600">
            ...
            Activate Bus {busRegistration}
          </DialogTitle>
          <DialogDescription>
            This action will activate the bus and make it available for service. Please review the changes below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-green-200 bg-green-50">
          ...
          <AlertDescription className="text-green-800">
            <strong>Notice:</strong> Activating this bus will make it available for upcoming trips and operations.
          </AlertDescription>
        </Alert>
