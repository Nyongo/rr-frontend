...
interface BusDeactivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export const BusDeactivationDialog = ({ 
  isOpen, 
  onClose, 
  busRegistration, 
  onConfirm 
}: BusDeactivationDialogProps) => {
  ...
  const [deactivationItems, setDeactivationItems] = useState<DeactivationItem[]>([
    {
      id: 'bus',
      label: 'Bus',
      description: 'The bus will not be available to be assigned to the upcoming trips by the minders.',
      status: 'pending'
    }
  ]);
  ...
  const handleDeactivate = async () => {
    ...
    toast({
      ...
      description: `Bus ${busRegistration} has been successfully deactivated and won't be available for the upcoming trips.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Deactivating Bus {busRegistration}</DialogTitle>
            <DialogDescription>
              Please wait while we deactivate the bus and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-red-600">
            ...
            Deactivate Bus {busRegistration}
          </DialogTitle>
          <DialogDescription>
            This action will deactivate the bus and affect related operations. Please review the impact below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Warning:</strong> Deactivating this bus will affect ongoing operations and scheduled trips.
          </AlertDescription>
        </Alert>
