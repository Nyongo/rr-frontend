...
interface DriverDeactivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  driverName: string;
  onConfirm: () => void;
...
export const DriverDeactivationDialog = ({ 
  isOpen, 
  onClose, 
  driverName, 
  onConfirm 
}: DriverDeactivationDialogProps) => {
  ...
  const [deactivationItems, setDeactivationItems] = useState<DeactivationItem[]>([
    {
      id: 'driver',
      label: 'Driver',
      description: 'Drivers wont be able to login to their account and not be able to be assigned to the next trips only.',
      status: 'pending'
    }
  ]);
  ...
  const handleDeactivate = async () => {
    ...
    toast({
      ...
      description: `Driver ${driverName} has been successfully deactivated and removed from active service.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Deactivating Driver {driverName}</DialogTitle>
            <DialogDescription>
              Please wait while we deactivate the driver and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-red-600">
            ...
            Deactivate Driver {driverName}
          </DialogTitle>
          <DialogDescription>
            This action will deactivate the driver and affect related operations. Please review the impact below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Warning:</strong> Deactivating this driver will affect ongoing operations and scheduled trips.
          </AlertDescription>
        </Alert>
