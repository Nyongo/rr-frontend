...
interface MinderDeactivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  minderName: string;
  onConfirm: () => void;
...
export const MinderDeactivationDialog = ({ 
  isOpen, 
  onClose, 
  minderName, 
  onConfirm 
}: MinderDeactivationDialogProps) => {
  ...
  const [deactivationItems, setDeactivationItems] = useState<DeactivationItem[]>([
    {
      id: 'minders',
      label: 'Minder',
      description: 'Minders wont be able to login to their account and manage their next trips.',
      status: 'pending'
    }
  ]);
  ...
  const handleDeactivate = async () => {
    ...
    onConfirm();
    toast({
      title: "Minder Deactivated!",
      description: `Minder ${minderName} has been successfully deactivated and removed from active service.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Deactivating Minder {minderName}</DialogTitle>
            <DialogDescription>
              Please wait while we deactivate the minder and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-red-600">
            ...
            Deactivate Minder {minderName}
          </DialogTitle>
          <DialogDescription>
            This action will deactivate the minder and affect related operations. Please review the impact below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Warning:</strong> Deactivating this minder will affect ongoing operations and scheduled trips.
          </AlertDescription>
        </Alert>
