...
interface MinderActivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  minderName: string;
  onConfirm: () => void;
...
export const MinderActivationDialog = ({ 
  isOpen, 
  onClose, 
  minderName, 
  onConfirm 
}: MinderActivationDialogProps) => {
  ...
  const [activationItems, setActivationItems] = useState<ActivationItem[]>([
    {
      id: 'minders',
      label: 'Minder',
      description: 'Minders will be able to login to their account and be able to manage their next trips.',
      status: 'pending'
    }
  ]);
  ...
  const handleActivate = async () => {
    ...
    onConfirm();
    toast({
      title: "Minder Activated!",
      description: `Minder ${minderName} has been successfully activated and is now available for service.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Activating Minder {minderName}</DialogTitle>
            <DialogDescription>
              Please wait while we activate the minder and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-green-600">
            ...
            Activate Minder {minderName}
          </DialogTitle>
          <DialogDescription>
            This action will activate the minder and make them available for service. Please review the changes below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-green-200 bg-green-50">
          ...
          <AlertDescription className="text-green-800">
            <strong>Notice:</strong> Activating this minder will make them available for upcoming trips and operations.
          </AlertDescription>
        </Alert>
