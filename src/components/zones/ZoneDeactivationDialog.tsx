...
interface ZoneDeactivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  zoneName: string;
  onConfirm: () => void;
...
export const ZoneDeactivationDialog = ({ 
  isOpen, 
  onClose, 
  zoneName, 
  onConfirm 
}: ZoneDeactivationDialogProps) => {
  ...
  const [deactivationItems, setDeactivationItems] = useState<DeactivationItem[]>([
    {
      id: 'zones',
      label: 'Zone',
      description: 'The zone wont be available to use in the upcoming trips.',
      status: 'pending'
    }
  ]);
  ...
  const handleDeactivate = async () => {
    ...
    onConfirm();
    toast({
      title: "Zone Deactivated!",
      description: `Zone ${zoneName} has been successfully deactivated and will not be available for upcoming trips.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Deactivating Zone {zoneName}</DialogTitle>
            <DialogDescription>
              Please wait while we deactivate the zone and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-red-600">
            ...
            Deactivate Zone {zoneName}
          </DialogTitle>
          <DialogDescription>
            This action will deactivate the zone and affect related operations. Please review the impact below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Warning:</strong> Deactivating this zone will affect next trips and scheduled operations.
          </AlertDescription>
        </Alert>
