...
interface ZoneActivationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  zoneName: string;
  onConfirm: () => void;
...
export const ZoneActivationDialog = ({ 
  isOpen, 
  onClose, 
  zoneName, 
  onConfirm 
}: ZoneActivationDialogProps) => {
  ...
  const [activationItems, setActivationItems] = useState<ActivationItem[]>([
    {
      id: 'zones',
      label: 'Zone',
      description: 'The zone will be available to use in the next trips.',
      status: 'pending'
    }
  ]);
  ...
  const handleActivate = async () => {
    ...
    toast({
      ...
      description: `Zone ${zoneName} has been successfully activated and is now available for upcoming trips.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Activating Zone {zoneName}</DialogTitle>
            <DialogDescription>
              Please wait while we activate the zone and update all related systems...
            </DialogDescription>
          </DialogHeader>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-green-600">
            ...
            Activate Zone {zoneName}
          </DialogTitle>
          <DialogDescription>
            This action will activate the zone and make it available for service. Please review the changes below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-green-200 bg-green-50">
          ...
          <AlertDescription className="text-green-800">
            <strong>Notice:</strong> Activating this zone will make it available for next trips and operations.
          </AlertDescription>
        </Alert>
