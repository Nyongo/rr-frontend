...
export const UserDeactivationDialog = ({
  ...
  const [deactivationItems, setDeactivationItems] = useState<DeactivationItem[]>([
    {
      id: 'user',
      label: 'User',
      description: 'This will log out the user\'s session from the system, and they will not be able to log in to their account.',
      status: 'pending'
    }
  ]);
  ...
  const handleDeactivate = async () => {
    ...
    toast({
      ...
      description: `User ${userName} has been successfully deactivated and won't be able to access their account.`,
    });
    onClose();
    resetDialog();
  };
    ...
    return (
      <Dialog open={isOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Deactivating User {userName}</DialogTitle>
            <DialogDescription>
              Please wait while we deactivate the user and update all related systems...
            </DialogDescription>
          </DialogHeader>
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            This action will deactivate the user and affect their access. Please review the impact below.
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Warning:</strong> Deactivating this user will affect their access to the system.
          </AlertDescription>
        </Alert>
